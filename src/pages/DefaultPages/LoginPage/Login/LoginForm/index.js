        ...
        switch (command) {
          case 'UserLogon': {
            if (temp_array.length === 8) {
              ...
              let permission_info = "<UserCheckPermission><" + temp_array[5].slice(0, -1) + "><" + temp_array[6].slice(0, -1) + "><Modify System Security Level>";
              checkPermission_flag = 0;
          ...
          case 'GetSystemInfo': {
            ...
            console.log("Security Level", received_msg);
            if (temp_array.length === 7) {
              systemSecurityLevel = temp_array[4].slice(0, -1);
            }
            dispatch({
          ...
          case 'UserCheckPermission': {
            if (temp_array.length === 6) {
                if(checkPermission_flag === 0) {
                    let result = temp_array[4].slice(0, -1);
                    if(result === "OK") {
                        console.log("UserPermission: ", "Modify System Security Level")
                        cookie.save('UserPermission', "Modify System Security Level");
                        checkPermission_flag = -1;
                    } else {
                        let permission_info = "<UserCheckPermission><" + temp_array[5].slice(0, -1) + "><" + temp_array[6].slice(0, -1) + "><Modify Camera Motion Detection>";
                        checkPermission_flag = 1;
                        this.ws.send(permission_info);
                    }
                }
                if(checkPermission_flag === 1) {
                    let result = temp_array[4].slice(0, -1);
                    if(result === "OK") {
                        console.log("UserPermission: ", "Modify Camera Motion Detection")
                        cookie.save('UserPermission', "Modify Camera Motion Detection");
                        checkPermission_flag = -1;
                    }
                }
            }
      ...
      if (temp_array.length > 1) {
        ...
        switch (command) {
          ...
          case 'GetSystemInfo': {
            let systemSecurityLevel = 1;
            if (temp_array.length === 7) {
              systemSecurityLevel = temp_array[3].slice(0, -1);
            }
            dispatch({
              type: 'SET_System_Security_Level',
              systemSecurityLevel: systemSecurityLevel,
